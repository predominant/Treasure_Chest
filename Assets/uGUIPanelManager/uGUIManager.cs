        ...
        void MovePanels()
        {
            ...
            for (i = 0; i < length; i++)
            {
                ...
                else
                {
                    ...
                    {
                        if (mp.panel.duration == 0f || mp.instant)
                        {
                            if (!mp.additional && mp.targetState != PanelState.Hide)
                            {
                                //hide other panels
                        ...
                        mp.panel.CachedCanvasGroup.alpha = mp.panel.stateTransform [(int)GetToggleState(mp)].alpha;



                        mp.panel.panelState = GetToggleState(mp);
                        if (mp.instant || mp.panel.duration == 0f)
                        {
                    ...
                    else
                    {
                        ...
                        mp.panel.CachedRectTransform.localScale = Vector3.Lerp(mp.panel.stateTransform [(int)mp.panel.panelState].scale,
						                                                       ...
						                                                       evaluate);
                        mp.panel.CachedCanvasGroup.alpha = Mathf.Lerp(mp.panel.stateTransform [(int)mp.panel.panelState].alpha,
                                                                      mp.panel.stateTransform [(int)GetToggleState(mp)].alpha,
                                                                      evaluate);
...
//                HidePanelInst(mp.name, false, true);
            ...
            foreach (uGUIManagedPanel mp in managedPanels)
            {
                ...
                if (mp.isMainPanel)
                {
                    ...
                    if (count == 0)
                    {
                        SetPanelStateInst(mp.name, PanelState.Show, true, true, this.Settings.showMainPanelInstantOnSceneLoad);
                    }
                    else
                    {
                        SetPanelStateInst(mp.name, PanelState.Show, true, false, this.Settings.showMainPanelInstantOnSceneLoad);
                    }
                    count ++;
                }
