...
{
    public partial class InventoryCreateNewItemEditor : EditorWindow
    {

        //private Editor itemPicker;
        private InventoryItemTypePicker picker { get; set; }
        private int step = 0;
        ...
        {
            var window = EditorWindow.GetWindow<InventoryCreateNewItemEditor>(true, "Create a new item", true);
        ...
        public void Reset()
        {
            ...
            picker = InventoryItemTypePicker.Get();
            picker.Show(true);
            picker.Close(true);
        ...
        public void OnGUI()
        {
            ...
            if(forceFocus)
                EditorWindow.FocusWindowIfItsOpen<InventoryCreateNewItemEditor>();
        ...
        public void Step2()
        {
            ...
            var style = new GUIStyle(InventoryEditorStyles.boxStyle);
            ...
            if (GUILayout.Button("2D sprite with trigger", GUILayout.ExpandWidth(true), GUILayout.Height(30)))
            {
                ...
                step2Model.AddComponent<SpriteRenderer>();
                var col = step2Model.AddComponent<BoxCollider2D>();
                col.isTrigger = true;
            }
                        ...
                        if (DragAndDrop.objectReferences.Length == 1)
                        {
                            ...
                            else
                            {
                                if (sprite != null)
                                {
                                    ...
                                    step2Model = new GameObject("2D Sprite");
                                    var spr = step2Model.AddComponent<SpriteRenderer>();
                                    spr.sprite = sprite;
