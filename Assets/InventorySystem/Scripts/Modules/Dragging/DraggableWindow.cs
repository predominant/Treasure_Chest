...
namespace Devdog.InventorySystem.UI
{
    ...
    {
        [Header("Dragging")] public float dragSpeed = 1.0f;


        [Header("Bounds")] public bool preventFromGoingOutOfBounds = true;
        public Vector2 spacing = new Vector2(100.0f, 100.0f);

        /// <summary>
        /// Once clicked should this draggable window be moved to the foreground?
        /// </summary>
        [Header("Bring to foreground")] public bool onClickBringToForeground = true;
        ...
        public void Awake()
        {
            ...
            var w = r.rect.width * r.pivot.x;
            var h = r.rect.height * r.pivot.y;
            minPos = new Vector2(w - spacing.x, h - spacing.y);
            maxPos = new Vector2(Screen.width + w - spacing.x, Screen.height + h - spacing.y);
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            if (preventFromGoingOutOfBounds)
                transform.position = Clamp(transform.position, minPos, maxPos);
        }
